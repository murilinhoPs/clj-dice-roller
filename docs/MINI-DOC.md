...
7. **2025-10-03** — visão de app RPG em `docs/dice-roller-rpg-idea.md` (ainda não implementada).
...
| Caminho | Papel |
|---|---|
| `project.clj` | Lein, Clojure 1.11.1, `spec.alpha`, Transcriptor; `:repl-options` inicia em `clj-dice-roller.core`; alias `repls-test`; profile `:dev` inclui `repls` em `:source-paths` |
...
| `docs/*` | Caderno de estudo (`data-structure.md`, `validation.md`) + ideia futura de produto RPG (`dice-roller-rpg-idea.md`) |
...
- Parse de strings tipo `"3d6"` (e `"d4"` como um dado) via `parse-roll`.
- Rolagem com `rand-int` e modifiers (`+ - * /`). `roll` está marcada `^:private` (o `.repl` ainda a chama).
- Várias expressões somadas (`roll-multiple`).
- Manter maior / menor resultado (`roll-keep-highest` / `roll-keep-lowest`).

Há um TODO no `core` para unificar a escolha entre roll / multiple / vantagem. Prefira as formas públicas (`roll-multiple`, `roll-keep-highest`, `roll-keep-lowest`) nos exemplos abaixo; o Transcriptor ainda exercita `dice/roll`. Alguns comentários antigos no código ainda mostram assinaturas experimentais.
...
Alias em `project.clj`: `["run" "-m" "repl-runner"]`. O runner (`repls/repl_runner.clj`) executa cada `*.repl` em `./repls` com `(xr/run …)` e o arquivo valida com `(xr/check! ...)`.
